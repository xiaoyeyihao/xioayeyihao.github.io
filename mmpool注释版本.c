...
struct mp_node_s {
	...
	unsigned char *last;//4K内存中可用空间的首地址，内存页空闲空间最开始的位置。
	unsigned char *end;// 4k内存块的最后位置     内存页最末尾的位置
	
	struct mp_node_s *next;
	size_t failed; //当failed > 4 时 current 跳到下个内存页节点   failed值的是该内存页分配小块内存由于空间不够失败的次数
};

struct mp_pool_s {

	size_t max;  //区分大块和小块的一个临界值
