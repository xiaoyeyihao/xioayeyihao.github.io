...
struct ethhdr {
	unsigned char h_dst[ETH_ADDR_LENGTH];//目的mac地址
	unsigned char h_src[ETH_ADDR_LENGTH];//原mac地址
	unsigned short h_proto;//协议类型 (ip协议还是arp协议)
...
struct iphdr {
	...
	unsigned char hdrlen:4,
				  version:4; // 0x45 //版本号和首部长度
				  

	unsigned char tos;   //服务类型

	unsigned short totlen;  //总长刀研
